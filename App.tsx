...
import { fetchIndustryAnalysis, generatePodcastSpeech, decodeAudio, createAudioBlob } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const handlePlayPodcast = async () => {
    ...
    try {
      ...
      const audioText = `${result.summary}. Temas de hoy: ${result.sections.map(s => `${s.title}: ${s.content}`).join('. ')}`;
      const base64Audio = await generatePodcastSpeech(audioText);
  ...
  const handleDownloadAudio = async () => {
    ...
    setExporting(true);
    try {
      const audioText = `${result.summary}. Detalle: ${result.sections.map(s => `${s.title}: ${s.content}`).join('. ')}`;
      const base64Audio = await generatePodcastSpeech(audioText);
      if (base64Audio) {
        ...
        a.href = url;
        a.download = `Podcast_Visual_Industry_${new Date().toISOString().split('T')[0]}.wav`;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <header className="text-center mb-16">
        ...
        <p className="text-lg text-slate-600 max-w-2xl mx-auto">
          Análisis semanal inteligente impulsado por IA. Escucha nuestro podcast generado automáticamente.
        </p>
      </header>
      ...
      <div className="flex flex-col items-center justify-center gap-6 mb-12">
        ...
        >
          {loading ? 'Analizando Tendencias...' : 'Generar Análisis Semanal'}
        </button>
        ...
        {result && (
          <div className="flex flex-wrap justify-center gap-4">
            ...
            >
              ...
              {playing ? 'Detener Podcast' : 'Escuchar Podcast (Joe & Jane)'}
            </button>
        ...
        <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
          <section className="bg-indigo-900 rounded-3xl p-8 text-white relative overflow-hidden">
            ...
            <div className="relative z-10">
              <h2 className="text-2xl font-bold mb-4 flex items-center gap-2">✨ Resumen Ejecutivo</h2>
              <p className="text-xl text-indigo-100 leading-relaxed font-light">{result.summary}</p>
            </div>
          </section>
          ...
          {result.sources.length > 0 && (
            <footer className="mt-16 pt-8 border-t border-slate-200">
              <h4 className="text-sm font-semibold text-slate-500 uppercase tracking-widest mb-4">Fuentes</h4>
              <div className="flex flex-wrap gap-4">
                {result.sources.map((source, idx) => (
                  <a key={idx} href={source.web.uri} target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-lg text-sm transition-colors">
                    {source.title}
                  </a>
                ))}
              </div>
